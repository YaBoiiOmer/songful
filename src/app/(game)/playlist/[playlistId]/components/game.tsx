      ...
      <Card className={cn("w-full max-w-lg", isGameOver && "pointer-events-none")}>
        ...
        <CardContent className="flex flex-col gap-4 items-center w-full">
          ...
          <SearchBar
            songs={songs}
            onGuess={(guess) => {
              setGuesses((prev) => [...prev, { name: guess.name, isCorrect: guess.id === randomSong?.id }]);
              if (guess.id === randomSong?.id) {
                setIsGameOver(true);
          ...
          />
          <CustomAudioPlayer
            audioUrl={randomSong.cloudinaryUrl}
    ...
    <div className="absolute top-0 left-0 w-full h-full bg-black/40 flex items-center justify-center">
      <Card className="w-full max-w-lg animate-fade-in-up duration-500 ">
        <CardHeader>
          ...
          <CardDescription className="text-center">
            You guessed <span className="font-bold text-white">{formatSongName(song)}</span> in{" "}
            <span className={`font-bold ${getStageColor(stage, "text")}`}>{guesses.length}</span> guesses.
          </CardDescription>
        </CardHeader>
